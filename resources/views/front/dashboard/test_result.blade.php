...
#test_questions li a{
  ...
  .wrong_answered
  {
                        ...
                        <!--     <div class="input-group input-timer-group"><div class="input-group-addon"></div><timer-directive interval="1000" countdown="sectionRemainingTime_sec" finish-callback="userOutOfTime()" class="input-timer form-control ng-binding ng-isolate-scope"><span class="ng-binding ng-scope">00:03:03</span></timer-directive></div> -->
                            <div class="col-lg-5 col-sm-12 col-md-5">
                                <div class="register-login-area stop-pause">
                                 <a href="{{ url('analysis/'.Request::segment(2).'/'.Request::segment(3).'/'.Request::segment(4))}}" class="btn active" style="height: 31px;line-height: 31px;font-size: 12px;margin-top: 35px; ">Back to Results Summary</a>
                             </div>
                            </div>
                        </div>
                                           ...
                                           <div class="page_number">
                                            ...
                                            <ul id="test_questions" >
                                                    ...
                                                    <li>                                                        
                                                        ...
                                                        ?>  
                                                        onclick="javascript:get_question({{ $rowqst->question_count }})">
                                                            {{ $counts }}
                                                        </a>
                                                    </li>
                                                    <?php $counts++; ?>
                                                ...
                                                @if($qst->question_type_id==2)
                                                 <div class="col-md-6 col-sm-6 col-lg-5 ans_div">
                                                    ...
                                                    if($key!=0)
                                                    {
                                                        ...
                                                        ?>
                                                        <a href="#" class="btn clever-btn previous" onclick="javascript:previous({{$previousqstid}})">Previous Question</a>
                                                    <?php } } ?>
                                                    <?php 
                                                    $key = array_search(Request::segment(5), $next_array);
                                                    if (array_key_exists($key+1,$next_array)) 
                                                    ...
                                                    ?>
                                                    <a href="#" class="btn clever-btn" onclick="javascript:next({{$nextqstid}})">Next Question</a>
                                                    <?php } ?>
                                                    </center><br>
                                                    <div id="answer_div">
                                                    <ul>
                                                        ...
                                                        <li>
                                                            ...
                                                            ?> stroke="#eeeeee" stroke-width="4" class="circle_ans" id="circle{{$ans->id}}" />
                                                            <text 
                                                            <?php 
                                                            if($ans->id == $qst->user_answer_id) 
                                                                echo 'fill="#ffffff"'; 
                                                                ...
                                                                echo 'fill="#000000"';
                                                             ?> 
                                                             id="text{{$ans->id}}" font-size="15" x="26" class="text_ans" y="38">{{ chr($sn) }}</text>
                                                            </svg>
                                                            <label style="display: flex; margin-right: 10px; color: #ddd"></label>
                                                            <label style="display: flex;margin-top: 4%;">{{ $ans->answer }}</label>
                                                            <div style="clear: both;"></div>
                                                        </li>
                                                        <?php $sn++; ?>
                                                        @endforeach
                                                 ...
                                                 <div class="col-12 col-lg-12" style="margin-top: 10px;"> 
                                                    <textarea class="form-control" name="essay_ans" id="essay_ans" disabled="disabled" placeholder="Please write your answer here." rows="15">{{$qst->user_answer}}</textarea>
                                                 </div>
                                                 @endif
                                                 <br>
                                                 <div style="clear: both;"></div>
    ...
    <input type="hidden" name="taken_test_id" id="taken_test_id" value="{{ $test_id }}">
    <!-- ##### Courses Content End ##### -->
@endsection
...
<script>
    ...
    {
        var url = "{{ Request::root().'/'.Request::segment(1).'/'.Request::segment(2).'/'.Request::segment(3).'/'.Request::segment(4).'/'}}"+nextqstid+"{{'/'.Request::segment(6).'/'.Request::segment(7)}}";
        console.log(url);
    ...
    function previous(previousqstid)
        {
            var url = "{{ Request::root().'/'.Request::segment(1).'/'.Request::segment(2).'/'.Request::segment(3).'/'.Request::segment(4).'/'}}"+previousqstid+"{{'/'.Request::segment(6).'/'.Request::segment(7)}}";
            console.log(url);
            window.location.replace(url);
        }
    ...
    {
        var url = "{{ Request::root().'/'.Request::segment(1).'/'.Request::segment(2).'/'.Request::segment(3).'/'.Request::segment(4).'/'}}"+id+"{{'/'.Request::segment(6).'/'.Request::segment(7)}}";
        console.log(url);
        window.location.replace(url);
    }
